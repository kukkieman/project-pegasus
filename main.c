...
// ------ DEFINITIONS ------
#define WINDOW_TITLE                                                         "ProjectPegasus"
...
void drawGame(){
    ...
    // left side
		DrawLineEx(
      (Vector2) { LEFT_MARGIN + (line * LINE_WIDTH) + (line * MARGIN_BETWEEN_LINE), 0 },
		...
		// right side
		DrawLineEx(
      (Vector2) { LEFT_MARGIN + LINE_WIDTH + (line * LINE_WIDTH) + (line * MARGIN_BETWEEN_LINE), 0 },
    ...
    DrawRectangle(touch_blocks[line].x,
      ...
      text,
			...
			TOUCH_BLOCK_FONT_SIZE, TOUCH_BLOCK_FONT_COLOR
    );
